        ...
        try:
            ...
            while running:
                ...
                if not frame_buffer.empty():
                    ...
                    frm = frame_buffer.get()
                    left_lines_fitted, right_lines_fitted = LaneDetection.LD(picture=frm)
                    if left_lines_fitted.shape == (2, 2):
                        if (left_lines_fitted[1][1] - left_lines_fitted[0][1]) < (-0.65) * (
                                left_lines_fitted[1][0] - left_lines_fitted[0][0]) - 0.01:
                            cv2.line(frm, tuple(left_lines_fitted[0]), tuple(left_lines_fitted[1]),
                                     color=(255, 0, 255),
                                     thickness=5)  # purple

                    if right_lines_fitted.shape == (2, 2):
                        if (right_lines_fitted[1][1] - right_lines_fitted[0][1]) > 0.65 * (
                                right_lines_fitted[1][0] - right_lines_fitted[0][0]) + 0.01:
                            cv2.line(frm, tuple(right_lines_fitted[0]), tuple(right_lines_fitted[1]),
                                     color=(0, 255, 255),
                                     thickness=5)  # yellow
                    if out_path is None:
                        cv2.imshow(path, frm)
